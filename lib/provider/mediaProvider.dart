...
class MediaProvider extends ChangeNotifier {
  ...
  Future<void> updateUIElements() async {
    String currentAlbumId =
        await AudioService.currentMediaItem.extras["albumId"];
    ...
    PaletteGenerator palette = await PaletteGenerator.fromImageProvider(
        FileImage(File(
            AudioService.currentMediaItem.artUri.replaceAll("file://", ""))));
  ...
  void loadSongList() async {
    ...
    for (SongInfo song in songInfoList) {
      File artworkFile =
          File("$thumbnailsPath${song.title.replaceAll("/", "_")}MQ.jpg");
      ...
      try {
        listMediaItems.add(MediaItem(
            ...
            extras: {"albumId": song.id, "artwork": artworkFile.path}));
      } catch (_) {}
    }
    notifyListeners();
  }
  ...
  Future<void> loadVideoList() async {
    ...
    Directory(extPath)
      ...
      if (
        ...
        if (listFolders.firstWhere(
                (element) => element.path == dirname(videoItem.path),
                orElse: () => null) ==
            null) {
