...
class DatabaseService {
  ...
  Future<SongFile> getDownload(String id) async {
    Database db = await database;
    List<Map> data = await db.query(table, where: 'id = ?', whereArgs: [id]);
    if (data.length > 0) {
  ...
  Future<List<SongFile>> getDownloadList() async {
    ...
    await Future.forEach(result, (element) async {
      ...
      if (await File(songFile.path).exists()) {
        ...
        File coverPath = File(
            "$thumbnailsPath/${songFile.title.replaceAll("/", "_")}MQ.jpg");
        if (!await coverPath.exists()) {
          File coverImage = await FFmpegExtractor.generateArtwork(
