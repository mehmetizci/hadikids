...
class TagsManager {
  ...
  static Future<int> writeArtwork({
    ...
    if (artworkPath != null && artworkUrl != null)
      throw Exception(
          "Both artworkPath and artworkUrl defined, please define only one");
    if (artworkPath != null) {
      if (!await File(artworkPath).exists())
        throw Exception("artworkPath provided path doesnt exist or is invalid");
    }
    String artwork;
    bool fromUrl;
    Directory directory;
    ...
    if (fromUrl == true) {
      ...
      if (response.statusCode != 200) {
        throw Exception(
            "Error downloading artwork, check your Url or internet connection");
      }
      await artworkFile.writeAsBytes(response.bodyBytes);
    }
    int result = await _channel.invokeMethod(
        "writeArtwork", {"songPath": songPath, "artworkPath": artwork});
    return result;
  }
  ...
  static Future<File> generateCover(String url) async {
    File artwork = new File((await getApplicationDocumentsDirectory()).path +
