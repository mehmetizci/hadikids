...
enum ArtworkExtractMethod { Automatic, AudioQuery, FFmpeg }

/// Select which type of [Artwork] return when extracting it from
/// the provided [AudioFile]
enum ExtractImageType { Artwork, Thumbnail }
...
class FFmpegExtractor {
  ...
  static Future<File> generateArtwork(
      {String audioFile,
      String audioId,
      ExtractImageType imageType = ExtractImageType.Artwork,
      ArtworkExtractMethod extractionMethod = ArtworkExtractMethod.Automatic,
      bool forceExtraction = false}) async {
    assert(audioFile != "" || audioFile != null);
    String artworkDir =
        (await getApplicationDocumentsDirectory()).path + "/Artworks/";
    if (!await Directory(artworkDir).exists())
      await Directory(artworkDir).create();
    File artwork = File(
        "$artworkDir${audioFile.split("/").last.replaceAll("/", "_")}HQ.jpg");
      ...
      if (result == 255 || result == 1) {
        ...
        try {
          Size size = imageType == ExtractImageType.Artwork
              ? Size(800, 800)
              : Size(250, 250);
          bytes = await FlutterAudioQuery()
              .getArtwork(type: ResourceType.SONG, id: audioId, size: size);
        } catch (_) {}
        if (bytes != null && bytes.isNotEmpty) {
          await artwork.writeAsBytes(bytes);
        } else {
          var assetBytes =
              await rootBundle.load('assets/images/artworkPlaceholder_big.png');
          await artwork.writeAsBytes(assetBytes.buffer
  ...
  static Future<File> getVideoThumbnail(File videoFile) async {
    ...
    String videoTitle = videoFile.path
